...
3. `npm run dist` and send **`release\Rift.lol-Setup-0.1.13.exe`** (the installer). Portable still works, but it cannot auto-update. Local `npm run dev` still uses `.env` (set `RIFT_USE_LOCAL_KEY=1` to skip the proxy).
...
Release tagged `v0.1.13` instead:

- `Rift.lol-Setup-0.1.13.exe`
- `Rift.lol-Setup-0.1.13.exe.blockmap`
- `latest.yml`
